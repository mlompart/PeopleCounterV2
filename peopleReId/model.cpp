...
namespace peopleReId {
    ...
    void Model::OnnxToTRTModel() {
        ...
        config->setMemoryPoolLimit(nvinfer1::MemoryPoolType::kWORKSPACE, 8_GiB);
        config->setFlag(nvinfer1::BuilderFlag::kFP16);

        std::cout << "start building engine" << std::endl;
        //auto data = builder->buildSerializedNetwork(*network, *config);
        //    nvinfer1::IRuntime *trtRuntime;
        //    engine = trtRuntime->deserializeCudaEngine(data, data->size());
    ...
    std::vector<float> Model::PreProcess(std::vector<cv::Mat> &vec_img) {
        ...
        for (const cv::Mat &src_img: vec_img) {
            ...
            cv::Mat flt_img;
            if (INPUT_CHANNEL == 1) cv::cvtColor(src_img, flt_img, cv::COLOR_RGB2GRAY);
            ...
            if (INPUT_CHANNEL == 1) flt_img.convertTo(flt_img, CV_32FC1, 1 / alpha);
            else if (INPUT_CHANNEL == 3)
            ...
            if (image_order == "BCHW") {
                ...
                for (int i = 0; i < INPUT_CHANNEL; ++i) split_img[i] = (split_img[i] - img_mean[i]) / img_std[i];
